...
#make directory where results will be stored
mkdir -p singularity_results

#get env vars and add to singularity docker image
#! First var binds $HOME/singularity_results to data location (change to desired dir) !#
...
SINGULARITY_BIND="/$HOME/singularity_results:/opt/openfoam7/tutorials/incompressible/icoFoam/elbow/" 
...
echo 'Starting Singularity Docker Instance...\n'

#run openfoam docker image within singularity instance
singularity instance start --writable-tmpfs --keep-privs --env-file $HOME/env.list docker://openfoam/openfoam7-paraview56 openfoam
